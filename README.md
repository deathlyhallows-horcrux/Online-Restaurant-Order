...
A web application for restaurant to view menu items, add items to cart and order. The pages are built using HTML, CSS, JavaScript, jQuery. The customer can login and add items to the cart to order. It consists of JSP pages and JDBC connections to MySQL database for Login and Registration.The customer's login details are checked with the stored database records for successful login else an error message will be displayed saying invalid login details.
